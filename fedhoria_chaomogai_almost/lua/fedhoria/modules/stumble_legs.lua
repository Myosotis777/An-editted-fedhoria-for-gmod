...
function MODULE:Init(phys_bone, lpos)
	...
	if(ent.citizen and citizen_hit:GetFloat()>0.5)then
		local hole = ents.Create("npc_info_target")
		hole:SetBody(ent)
		hole:Spawn()
	...
	if(ent.player and player_hit:GetFloat()>0.5)then
		timer.Simple(GetConVar("sv_fedhoria_facial_duaration"):GetFloat()/5,function()
		local hole = ents.Create("npc_info_target")
		hole:SetBody(ent)
	...
	if(ent.combine and combine_hit:GetFloat()>0.5)then
		local hole = ents.Create("npc_info_target_e")
		hole:SetBody(ent)
		--hole:Spawn()
		 for dir,ply in pairs (player:GetAll()) do
        hole:AddEntityRelationship(ply,D_HT,99)
	    --self:AddRelationship(player,D_LI,99)
        local npc_all=ents.FindByClass("npc_*")
          for key,x in pairs (npc_all) do
		  --print(x)
		    if IsValid(x) and x:IsNPC() then
	        if x:Disposition(ply)== "D_LI" or x:GetClass()=="npc_citizen" then   --like
	           x:AddEntityRelationship(hole,D_HT,99)
	           hole:AddEntityRelationship(x,D_HT,99)
	        else
	           hole:AddEntityRelationship(x,D_NU,99)
	           x:AddEntityRelationship(hole,D_NU,99)
	        end
			end
	      end
      end
